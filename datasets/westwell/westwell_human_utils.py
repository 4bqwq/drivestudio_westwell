...
from .westwell_sourceloader import (
    SMPLNODE_CLASSES,
    OPENCV2DATASET,
    AVAILABLE_CAM_LIST,
)
...
def project_human_boxes(
    ...
    valid_paths = [images_dir, extrinsics_dir, intrinsics_dir, instances_dir]
    for path in valid_paths:
        assert os.path.exists(path), \
            f"Path {path} does not exist, you need to run westwell preprocess to generate the necessary files"
    ...
    for cam_id in camera_list:
        ...
        for frame_id, frame_ins_list in frame_infos.items():
            ...
            # Load extrinsics for this frame and camera
            cam2world = np.loadtxt(os.path.join(extrinsics_dir, f"{frame_id:03d}_{cam_id}.txt"))
